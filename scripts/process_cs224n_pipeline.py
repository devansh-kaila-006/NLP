"""
Process CS224n NLP Video Lectures
"""
...
class CS224nPipelineProcessor(LoggerMixin):
    """Process CS224n NLP lectures"""
    ...
    def process_all_lectures(
        self,
        transcript_dir: str = "data/transcripts/cs224n",
        output_dir: str = "data/processed/video_chunks_cs224n"
    ):
        """Process all CS224n SRT files in directory"""
        transcript_dir = Path(transcript_dir)
        output_dir = Path(output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)

        # Get all SRT files (lectures and tutorials)
        srt_files = sorted(transcript_dir.glob("CS224n_*.srt"))

        self.logger.info(f"Found {len(srt_files)} CS224n lecture transcripts")
        ...
        self.logger.info(f"\n{'='*60}")
        self.logger.info(f"CS224N PROCESSING COMPLETE")
        ...
        for i, chunk in enumerate(all_chunks):
            # Determine if tutorial or lecture by checking source name
            source_name = chunk.video_source
            is_tutorial = any(tutorial_id in source_name for tutorial_id in ['T01', 'T02', 'T03'])
        ...
        # Save chunks
        chunks_path = output_dir / "cs224n_video_chunks.pkl"
        ...
        embeddings_path = output_dir / "cs224n_video_embeddings.npy"
        ...
        # Save FAISS index
        index_path = output_dir / "cs224n_video_index.faiss"
        metadata_path = output_dir / "cs224n_video_metadata.pkl"
...
def main():
    """Process all CS224n lectures"""
    processor = CS224nPipelineProcessor()

    print("="*80)
    print("CS224N NLP PIPELINE PROCESSOR")
    print("="*80)

    result = processor.process_all_lectures()

    print(f"\n{'='*80}")
    print("CS224N VIDEO PROCESSING COMPLETE!")
    print(f"{'='*80}")
    ...
    print(f"\n[SUCCESS] CS224n video pipeline ready for RAG integration!")
